...
export function PublicSettingsForm({
  ...
  return (
    <form action={action} className="space-y-4 p-4">
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-ink-800 bg-ink-850/50 p-3">
        <input
          type="checkbox"
          name="is_public"
        ...
        />
        <span>
          <span className="block text-sm font-medium text-ink-100">Publish this project</span>
      ...
      <fieldset disabled={!isPublic} className={isPublic ? '' : 'opacity-40'}>
        ...
        <div className="mt-3 rounded-lg border border-money-500/25 bg-money-500/5 p-3">
          <label className="flex cursor-pointer items-start gap-3">
            <input
              ...
              className="mt-0.5 h-4 w-4 shrink-0 rounded border-ink-700 bg-ink-900 text-money-500 focus:ring-money-500"
            />
            <span>
              <span className="block text-sm font-medium text-money-400">Revenue</span>
              <span className="mt-0.5 block text-xs text-ink-500">
          ...
          {showRevenue ? (
            <p className="mt-2 border-t border-money-500/20 pt-2 text-xs text-ink-500">
      ...
      {isPublic ? (
        <div className="border-t border-ink-850 pt-4">
          <p className="mb-2 text-xs font-medium text-ink-300">Embeddable badge</p>
